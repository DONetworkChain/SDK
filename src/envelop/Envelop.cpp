...
// void SdkImportKey(EVP_PKEY * pkeyPtr)
// {
//     std::string base58;
...
//     std::stringstream ss(std::string(reinterpret_cast<char*>(seedGetRead), PrimeSeedNum * 3 - 1)); // 
//     std::string hexPart;
//     for (int i = 0; i < PrimeSeedNum; i++) 
//     {
//         std::getline(ss, hexPart, '-');
//         uint8_t value;
//         std::from_chars(hexPart.c_str(), hexPart.c_str() + hexPart.size(), value, 16);
//         seedGetRead[i] = value;
//     }
...
//     sha256_hash(seedGetRead, sizeof(seedFull), outputArr);
